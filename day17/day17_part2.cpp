...
//                        }
        ...
        if (l.dirCount < 3)
        {
            ...
            else if (l.dir == DOWN && l.y < boardSizeY-1)
                neighbourDirections.push_back(DOWN);
            else if (l.dir == LEFT && l.x > 0)
                neighbourDirections.push_back(LEFT);
            else if (l.dir == RIGHT && l.x < boardSizeX-1)
                neighbourDirections.push_back(RIGHT);
        }
        else
        {
            if (l.x < boardSizeX-1 && !(l.dir == RIGHT && l.dirCount == 9) && l.dir != LEFT)
                neighbourDirections.push_back(RIGHT);
            if (l.x > 0 && !(l.dir == LEFT && l.dirCount == 9) && l.dir != RIGHT)
                neighbourDirections.push_back(LEFT);
            if (l.y > 0 && !(l.dir == UP && l.dirCount == 9) && l.dir != DOWN)
                neighbourDirections.push_back(UP);
            if (l.y < boardSizeY-1 && !(l.dir == DOWN && l.dirCount == 9) && l.dir != UP)
                neighbourDirections.push_back(DOWN);
        }
